app.controller('vantai', ['$scope', '$http', '$timeout', '$q', function ($scope, $http, $timeout, $q) {
        ...
        $scope.loadPageData = function (index) {
            ...
            $http.get(preUrl + "/phieu-giao-hang/load-list", {params: {search: search, offset: $scope.listData.numberPerPage * ($scope.listData.pageNumber - 1), number: $scope.listData.numberPerPage}})
                    .then(function (response) {
        ...
        $scope.xoaPhieu = function () {
            ...
            $http.post(preUrl + "/phieu-giao-hang/delete", vtToaHang, {headers: {'Content-Type': 'application/json'}})
                    .then(function (response) {
