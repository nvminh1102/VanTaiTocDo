app.controller('frameworkCtrl', ['$scope', '$http', 'fileUpload', function ($scope, $http, fileUpload) {
      ...
      $http.get(preUrl + "/bienNhan/nha-xe/search")
      ...
      $scope.search = function () {
        ...
        $http.get(preUrl + "/bienNhan/nha-xe/search", {params: {nhaXe: $scope.search.nhaXe, loaiXe: $scope.search.loaiXe, bienSo: $scope.search.bienSo, tenLaiXe: $scope.search.tenLaiXe}})
        .then(function (response) {
          if (response != null && response != 'undefined' && response.status == 200) {
            $scope.page = response.data;
            $scope.page.pageCount = getPageCount($scope.page);
      ...
      $scope.loadPage = function (pageNumber) {
        if (pageNumber >= 1) {
          $http.get(preUrl + "/bienNhan/nha-xe/search", {params: {p: pageNumber, numberPerPage: $scope.page.numberPerPage, nhaXe: $scope.search.nhaXe, loaiXe: $scope.search.loaiXe, bienSo: $scope.search.bienSo, tenLaiXe: $scope.search.tenLaiXe}})
          .then(function (response) {
            $scope.page = response.data;
            $scope.page.pageList = getPageList($scope.page);
            $scope.page.pageCount = getPageCount($scope.page);
  ...
  $scope.xoaNhaXe = function () {
    ...
    $http.post(preUrl + "/bienNhan/nha-xe/delete", param, {headers: {'Content-Type': 'application/json'}})
    .then(function (response) {
  ...
  $scope.addNhaXe = function () {
    ...
    var param = JSON.parse(JSON.stringify(call));
    $http.post(preUrl + "/bienNhan/nha-xe/them-moi-nha-xe", param, {headers: {'Content-Type': 'application/json'}})
    .then(function (response) {
      switch (Number(response.data)) {
        case 0:
