app.controller('frameworkCtrl',['$scope','$http','$filter','$window','$timeout','$q',function ($scope,$http,$filter,$window,$timeout,$q) {
    ...
    var urlSearch = "/system/sms-mt-queue/list/search";
    ...
    $scope.smsMTQueue={};
    $scope.smsMTQueue.status=0;
    $scope.smsMTQueue.message_type=0;
    $scope.SaveSmsMTQueue=function () {
        if($("#form-save").parsley().validate()){
             $('#modalXacNhan').modal('show');
        }
    }
    $scope.xacNhan=function () {
        var auctioneerJson = JSON.parse(JSON.stringify($scope.smsMTQueue));
        $http.post(preUrl + "/system/sms-mt-queue/addsmt", auctioneerJson, {headers: {'Content-Type': 'application/json'}})
            .then(function (response) {
