app.controller('frameworkCtrl',['$scope','$http','$filter','$window','$timeout','$q',function ($scope,$http,$filter,$window,$timeout,$q) {
    ...
    var urlSearch = "/system/sms-mt-log/list/search";
    ...
    $scope.loadListData=function () {
        $scope.searchvalue={};
        if(!isUndefined( $('#genlogmtfrom').val())){
            $scope.searchvalue.gen_date_from=stringToDate($('#genlogmtfrom').val(),'DD/MM/YYYY','/');
        }
        if(!isUndefined( $('#genlogmtto').val())){
            $scope.searchvalue.gen_date_to=stringToDate($('#genlogmtto').val(),'DD/MM/YYYY','/');

        }
        $scope.searchvalue.receiver_number=$scope.search.receiver_number;
        $scope.searchvalue.sender_number=$scope.search.sender_number;
        $scope.searchvalue.command_code=$scope.search.command_code;
        var search = JSON.stringify($scope.searchvalue);
        $http.get(preUrl + urlSearch, {params: {search:search,offset:0,number:$scope.listData.numberPerPage}})
            .then(function (response) {
                $scope.listData=response.data;
