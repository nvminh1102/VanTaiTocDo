app.controller('frameworkCtrl', ['$scope', '$http', 'fileUpload', function ($scope, $http, fileUpload) {
      ...
      $http.get(preUrl + "/bienNhan/search")
      ...
      $scope.search = function () {
        ...
        $http.get(preUrl + "/bienNhan/search", {params: {receiptCode: $scope.search.receiptCode, nameStock: $scope.search.nameStock, fromDateReceipt: $scope.search.fromDateReceipt, toDateReceipt: $scope.search.toDateReceipt}})
        .then(function (response) {
          if (response != null && response != 'undefined' && response.status == 200) {
            $scope.page = response.data;
            $scope.page.pageCount = getPageCount($scope.page);
      ...
      $scope.loadPage = function (pageNumber) {
        if (pageNumber >= 1) {
          $http.get(preUrl + "/bienNhan/search", {params: {p: pageNumber, numberPerPage: $scope.page.numberPerPage, receiptCode: $scope.search.receiptCode, nameStock: $scope.search.nameStock, fromDateReceipt: $scope.search.fromDateReceipt, toDateReceipt: $scope.search.toDateReceipt}})
          .then(function (response) {
            $scope.page = response.data;
            $scope.page.pageList = getPageList($scope.page);
            $scope.page.pageCount = getPageCount($scope.page);
  ...
  $scope.xoaBienNhan = function () {
    ...
    $http.post(preUrl + "/bienNhan/delete", param, {headers: {'Content-Type': 'application/json'}})
    .then(function (response) {
